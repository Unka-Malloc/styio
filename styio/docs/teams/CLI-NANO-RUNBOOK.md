...
51. In `styio-all-in-one`, `styio check --syntax --json --file` is the default IDE handoff from Vityo to the embedded compiler. It may accept `--config <path>` so the IDE can preserve project context and future config-driven symbol manifests, but it must remain syntax-only until Sema/type/lowering facts are explicitly exposed through a separate compiler-owned service contract.
