...
| 文件 | 摘要 |
|------|------|
| [`2026-04-07.md`](2026-04-07.md) | C.5 收口：state helper 内联克隆补齐 `MatchCases/Cases`，修复 `unsupported AST node in inlined state expression clone`，新增 `StateInlineMatchCasesFunctionUsesCallArgument` 回归并在 `styio_pipeline/security` 与 ASan/UBSan 下验证；D.3 新增 `StateInlineMatchCasesProgramLoop` 与 `soak_deep_state_inline_matchcases_program` 长跑门禁 |
