...
| 文件 | 摘要 |
|------|------|
| [`2026-04-07.md`](2026-04-07.md) | C.5 收口：state helper 内联克隆补齐 `MatchCases/Cases` 与 `InfiniteAST`，修复 `unsupported AST node in inlined state expression clone`，新增 `StateInlineMatchCasesFunctionUsesCallArgument` / `StateInlineInfiniteLiteralFunctionUsesCallArgument` 回归并在 `styio_pipeline/security` 与 ASan/UBSan 下验证；D.3 新增 `StateInlineMatchCasesProgramLoop`、`StateInlineInfiniteProgramLoop` 与对应 `soak_deep` 长跑门禁；E.6 将 parser 默认引擎切换到 `nightly` 并新增 `DefaultEngineIsNightlyInShadowArtifact` 契约回归，同时保留 `--parser-engine=new` 兼容别名；D.5 新增一键恢复脚本 `scripts/checkpoint-health.sh`，并将 `tests/milestones/m7` 的 shadow zero-fallback gate 正式纳入 CTest 与健康检查；E.7 继续压缩 parser fallback：`nightly` 子集原生接管 block/control 语句、表达式 postfix `?={...}`、资源 postfix `>> @file{...}` / `-> @file{...}`、name-led iterator / instant pull、`@[...]` snapshot/state-decl 起始、`@file... >> ...` 资源流入口，以及 `[ ... ] >> ...` list-start iterator 与换行边界；同时新增 `nightly_internal_legacy_bridges` 指标，把 iterator / match-cases / infinite-loop / forward body 的内部 legacy bridge 也纳入 shadow artifact 量化；新增 `_legacy/_nightly/_latest/_draft` 函数后缀规范 |
