...
| 文件 | 摘要 |
|------|------|
| [`2026-04-07.md`](2026-04-07.md) | C.5 收口：state helper 内联克隆补齐 `MatchCases/Cases` 与 `InfiniteAST`，修复 `unsupported AST node in inlined state expression clone`，新增 `StateInlineMatchCasesFunctionUsesCallArgument` / `StateInlineInfiniteLiteralFunctionUsesCallArgument` 回归并在 `styio_pipeline/security` 与 ASan/UBSan 下验证；D.3 新增 `StateInlineMatchCasesProgramLoop`、`StateInlineInfiniteProgramLoop` 与对应 `soak_deep` 长跑门禁；E.6 将 parser 默认引擎切换到 `new` 并新增 `DefaultEngineIsNewInShadowArtifact` 契约回归；D.5 新增一键恢复脚本 `scripts/checkpoint-health.sh` |
