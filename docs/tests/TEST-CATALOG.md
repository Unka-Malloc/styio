...
**Last updated:** 2026-04-04（新增 soak 分档 + D.5 最小化模板，并补 ParserEngine/ParserLookahead/NewParserExpr/NewParserStmt 回归测试）
...
| 目标 | 说明 | Automation |
|------|------|------------|
| `styio_test` | `tests/styio_test.cpp`：`StyioFiveLayerPipeline`、`StyioParserEngine`（legacy/new 一致性与非法引擎拒绝） | `ctest --test-dir build -L styio_pipeline` 或 `ctest --test-dir build -R '^StyioParserEngine\\.'` |
...
| 目标 | 说明 | Automation |
|------|------|------------|
| `styio_security_test` | `tests/security/styio_security_test.cpp`：lexer/Unicode/AST ownership/runtime，含 ParserLookahead trivia 回归、NewParserExpr 子集兼容回归、NewParserStmt print 子集回归 | `ctest --test-dir build -L security` |
