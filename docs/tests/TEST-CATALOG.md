...
**恢复一键检查：** `./scripts/checkpoint-health.sh --no-asan`（含 state-inline soak deep + `styio_pipeline` + `security` + `parser_shadow_gate_m7_zero_fallback`）。  
