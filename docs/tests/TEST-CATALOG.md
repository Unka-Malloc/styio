...
| 目标 | 说明 | Automation |
|------|------|------------|
| `styio_test` | `tests/styio_test.cpp`：`StyioFiveLayerPipeline`、`StyioParserEngine`（legacy/new 在 M1 算术与 typed bind 样例上一致，且非法引擎拒绝） | `ctest --test-dir build -L styio_pipeline` 或 `ctest --test-dir build -R '^StyioParserEngine\\.'` |
