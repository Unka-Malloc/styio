...
| 目标 | 说明 | Automation |
|------|------|------------|
| `styio_test` | `tests/styio_test.cpp`：`StyioFiveLayerPipeline`、`StyioParserEngine`（legacy/new 在 M1 算术、typed bind、compound assign 样例上一致，非法引擎拒绝，`--parser-shadow-compare` 可通过 M1 核心样例与全量样例集；`--parser-shadow-artifact-dir` 可产出 JSONL 记录并受参数约束回归覆盖；CI 通过 `scripts/parser-shadow-m1-gate.sh` 输出差异预算并写 `summary.json`） | `ctest --test-dir build -L styio_pipeline` 或 `ctest --test-dir build -R '^StyioParserEngine\\.'` |
...
| 目标 | 说明 | Automation |
|------|------|------------|
| `styio_security_test` | `tests/security/styio_security_test.cpp`：lexer/Unicode/AST ownership/runtime，含 ParserLookahead trivia 回归、NewParserExpr 子集兼容回归（含 compare/logic/dot-call token gate）、NewParserStmt（print/flex bind/final bind/compound assign/compare/logic/simple call/dot-call）子集回归与 Shadow fallback 回归（dot-chain） | `ctest --test-dir build -L security` |
