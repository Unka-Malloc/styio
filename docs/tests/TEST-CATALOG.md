...
| 目标 | 说明 | Automation |
|------|------|------------|
| `styio_soak_test` | `tests/soak/styio_soak_test.cpp`：长输入词法循环、文件句柄生命周期循环、文件/拼接 RSS 增长阈值守卫、非法句柄高频诊断循环、M6 流式程序重复执行、单参数 state helper（直返 `StateDecl`）内联链路长跑回归（输出 `1/3/6`）、state helper `MatchCases/Cases` 内联链路长跑回归（输出 `10/12/15`） | `ctest --test-dir build -L soak_smoke` |
